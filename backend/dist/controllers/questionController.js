...
export const addQuestionsToSession = async (req, res) => {
    try {
        ...
        if (!sessionId || !questions || !Array.isArray(questions)) {
            logger.warn(`Invalid input data for adding questions - Session: ${sessionId}`);
        ...
        await session.save();
        logger.info(`➕ ${createdQuestions.length} questions added to session: ${sessionId}`);
...
export const togglePinQuestion = async (req, res) => {
    try {
        ...
        await question.save();
        logger.info(`📌 Question ${question.isPinned ? "pinned" : "unpinned"}: ${req.params.id}`);
...
export const updateQuestionNote = async (req, res) => {
    try {
        ...
        question.note = note || "";
        await question.save();
        logger.info(`📝 Note updated for question: ${req.params.id} - Length: ${note?.length || 0} chars`);
        res.status(200).json({ success: true, question });
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : "Server Error";
